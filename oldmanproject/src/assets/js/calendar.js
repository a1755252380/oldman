...
;(function ($, window, document, undefined) {
  ...
  Calendar.prototype = {
    ...
    showCalendar: function () { // 输入数据并显示
      ...
      this.$calendarTitle_text.text(dateStr.substr(4, 2)+"月   "+year + '年' );
      ...
      this.$calendarDate_item.each(function (i) {
        ...
        if (returnDateStr(new Date()) === allDay_str) {
          $(this).attr('class', 'item item-curDay item-selected item-curMonth');
          $("#backToday").text(allDay_str.substr(6,8))
          $("#calendar_foot").text("今天是"+allDay_str.substr(0,4)+"年 "+allDay_str.substring(4,6)+"月 "+allDay_str.substr(6,8)+"日")
    ...
    renderDOM: function () { // 渲染DOM
      this.choosebtn=$('<div class="calendar-choosebtn"></div>')
      ...
      this.calendar_footer = $('<div class="calendar-foot" id="calendar_foot"></div>');
      ...
      var _dayStr = '<i class="triangle"></i>'+
                    '<p class="date"></p>'+
                    '<p class="week"></p>';
      var choosebtn_body= '<div class="qiwendiv"><span class="qiwentitle">今日气温</span><br><span class="qiwen">32°</span></div><a href="javascript:;" id="backToday"></a>'
      for (var i = 0; i < 6; i++) {
      ...
      this.choosebtn.html(choosebtn_body)
      this.$calendar.append( this.choosebtn,this.$calendar_week,this.$calendar_title, this.$calendar_date, this.$calendar_today,this.calendar_footer);
      this.$calendar.show();
    },
    ...
    inital: function () { // 初始化
      ...
      this.$calendarDate_item.click(function () {
        ...
        $("#calendar_foot").text("今天是"+_dateStr .substr(0,4)+"年 "+_dateStr .substring(4,6)+"月 "+_dateStr .substr(6,8)+"日")
